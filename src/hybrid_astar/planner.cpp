...
Planner::Planner() {
  ...
  std::cout << "YT: set up the hybridastar planner" << std::endl;
  // ___________________
  // TOPICS TO SUBSCRIBE
  if (Constants::manual) {
...
void Planner::setStartfromParam(const geometry_msgs::PoseWithCovarianceStamped::ConstPtr& initial) {
    //YT must convert x,y to the coordinate of binMap before start the algorithm
  float x = (initial->pose.pose.position.x - grid->info.origin.position.x) / grid->info.resolution;
  float y = (initial->pose.pose.position.y - grid->info.origin.position.y) / grid->info.resolution;
  ...
  if ( grid->info.height >= y && y >= 0 && grid->info.width >= x && x >= 0 ) {
    ...
    if (Constants::manual) { plan();}
    //std::cout << "setstartfromparam start x:" << x << " y:" << y << " t:" << Helper::toDeg(t) << std::endl;
    // publish start for RViz
    //pubStart.publish(startN);
  }
  else {
...
void Planner::setGoalfromParam(const geometry_msgs::PoseStamped::ConstPtr& end) {
  // retrieving goal position
  ...
  if ( grid->info.height >= y && y >= 0 && grid->info.width >= x && x >= 0 ) {
    validGoal = true;
    goal = *end;
//std::cout << "setgoalfromparam start x:" << x << " y:" << y << " t:" << Helper::toDeg(t) << std::endl;
    if (Constants::manual) { plan();}
...
//    const Node3D ori_nGoal(x, y, t, 0, 0, nullptr);
    ...
    // set theta to a value (0,2PI]
    t = Helper::normalizeHeadingRad(t);
//    Node3D ori_nStart(x, y, t, 0, 0, nullptr);
...
//    smoothedPath.updatePath(smoother.getPath());
    ...
    //std::cout << "YT: the path before adjust is: " << std::endl;
    //smoothedPath.PrintPath();
    ...
    {
        smoothedPath.setPath(i, smoothedPath.getPath().poses.at(i).pose.position.x*grid->info.resolution+ grid->info.origin.position.x,
                             smoothedPath.getPath().poses.at(i).pose.position.y*grid->info.resolution + grid->info.origin.position.y);
    }


    //std::cout << "YT: the startpoint is x = " << start.pose.pose.position.x << ", y = " << start.pose.pose.position.y << std::endl;
    //smoothedPath.PrintPath();
    //std::cout << "YT: the goalpoint is x = " << goal.pose.position.x << ", y = " << goal.pose.position.y << std::endl;
